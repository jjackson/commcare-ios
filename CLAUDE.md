...
**Phase 9: End-to-End UI Testing — Waves 0-4c COMPLETE, Waves 5-11 open.** Distinct successor to Phase 8 that closes the UI-level E2E gap AND picks up the relocated Phase 8 live-Connect test tail. The `+7426` Connect ID test prefix is used with a Playwright-driven OTP fetch against `https://connect.dimagi.com/users/connect_user_otp/` (persisted Dimagi SSO session) — the originally-planned OAuth2 client credentials were never needed. **Wave 0** (infrastructure + relocated Phase 8 live-Connect tests), **Wave 1** (Connect ID recovery flow), **Wave 2** (post-recovery state + session persistence), **Wave 3** (CommCare app install + login), **Wave 4a** (module list → case list navigation), **Wave 4b** (form navigation + back, no submit), and **Wave 4c** (full form fill + submit + HQ verify) are all shipped and reproducible via `.maestro/scripts/run-*.sh` orchestrators. Fixture user `+74260000042` (Hal Test) is registered in connect-id prod. Wave 3+ installs `Bonsaaso Application` from `jonstest` domain on HQ prod and logs in as `haltest@jonstest.commcarehq.org`. Wave 4c proved iOS can complete and submit a real 14-page XForm to HQ — required three serialization fixes (root xmlns via manual startTag write; nested namespace auto-declare in IosXmlSerializer; imePadding so IME doesn't cover form buttons). Product bugs caught and fixed during Phase 9: keychain bridging crash (Wave 0), recovery-endpoint misuse in new-reg flow (Wave 0), empty-photo rejection (Wave 0), form buttons hidden under keyboard (#394, Wave 4b), form XML missing xmlns + case namespace (Wave 4c) — see `docs/learnings/2026-04-08-phase9-ios-connect-id-bugs.md` and `docs/superpowers/plans/2026-04-09-phase9-wave4c-form-submit.md`. Bugs caught and filed (not yet fixed): state inconsistency between SetupScreen and Connect menu post-recovery (#389, Wave 2); `LoginViewModel.resolveDomain()` falls back to hardcoded `"demo"` for short-form usernames (#391, Wave 3). Wave 2 deferred PIN login + biometric tests because iOS has no lockscreen to trigger them. **Waves 5-11** remain open: form features deep dive, multi-app, marketplace, sync, settings, edge cases, reliability. See `docs/superpowers/specs/2026-04-08-phase9-e2e-ui-testing-design.md`.

**If you are an agent starting a session:** Phase 9 is the active phase. Waves 0-4c are complete. The full "install → sync → form → submit → HQ verify" chain works end-to-end against `jonstest` prod. If continuing Phase 9, read `docs/phase9/fixture-user.md` for the orchestrator workflow, then pick the next wave from the spec §7 summary. Wave 5 (form features deep dive) or Wave 6 (multi-app management) are the natural next steps.
...
- **Phase 9 Wave 2 plan**: `docs/superpowers/plans/2026-04-08-phase9-wave2-login-variants.md` — post-recovery SetupScreen state, session persistence across kill+relaunch, Connect menu entry (with state inconsistency bug #389 captured as a deliberately-failing assertion). PIN login and biometric deferred — no lockscreen exists to trigger them.
- **Phase 9 Wave 3 plan**: `docs/superpowers/plans/2026-04-08-phase9-wave3-app-install.md` — CommCare app install + mobile worker login. Fresh install → install Bonsaaso via profile URL → log in → home screen with Start button. Uses `jonstest` domain on HQ prod. Install-from-HQ-list and update detection deferred to Wave 3b/3c.
- **Phase 9 Wave 4a plan**: `docs/superpowers/plans/2026-04-08-phase9-wave4a-module-navigation.md` — module list + case list navigation baseline. Wave 3 setup + home → Start → Household List → Visit → Select Case (empty) → back → back → home. Form entry + submit deferred to Wave 4b.
- **Phase 9 Wave 4b plan**: `docs/superpowers/plans/2026-04-09-phase9-wave4b-form-navigation.md` — form navigation (no submit). Walks into Register Household, answers p1-p3 cascading radios, skips p4 optional location, asserts p5 Household Head Health ID text field, back-traces out. Full submit deferred to Wave 4c, blocked on #394.
- **Phase 9 Wave 4c plan**: `docs/superpowers/plans/2026-04-09-phase9-wave4c-form-submit.md` — full 14-page Register Household fill + submit + HQ API verify. Fixed iOS XML serialization bugs (root xmlns + nested case namespace) that blocked any form submission from reaching HQ prior to this wave.
...
- **iOS Connect ID / K/N interop**: `phase9-ios-connect-id-bugs` (2026-04-08) — three bugs caught by Phase 9's first E2E run: K/N `mapOf as CFDictionaryRef` crashing in Compose onClick on Xcode 26.4, new-reg calling recovery-only endpoint, `complete_profile` rejecting empty photo
