...
**Phase 9: End-to-End UI Testing — Waves 0, 1, and 2 COMPLETE, Waves 3-11 open.** Distinct successor to Phase 8 that closes the UI-level E2E gap AND picks up the relocated Phase 8 live-Connect test tail. The `+7426` Connect ID test prefix is used with a Playwright-driven OTP fetch against `https://connect.dimagi.com/users/connect_user_otp/` (persisted Dimagi SSO session) — the originally-planned OAuth2 client credentials were never needed. **Wave 0** (infrastructure + relocated Phase 8 live-Connect tests), **Wave 1** (Connect ID recovery flow), and **Wave 2** (post-recovery state + session persistence) are all shipped and reproducible via `.maestro/scripts/run-*.sh` orchestrators. Fixture user `+74260000042` (Hal Test) is registered in connect-id prod and verified end-to-end. Three real iOS product bugs were caught during Wave 0 execution and fixed (keychain bridging crash, recovery-endpoint misuse in new-reg flow, empty-photo rejection) — see `docs/learnings/2026-04-08-phase9-ios-connect-id-bugs.md`. A fourth bug (state inconsistency between SetupScreen and Connect menu post-recovery) was caught by Wave 2 scouting and filed as #389. Wave 2 also deferred PIN login + biometric tests because iOS has no lockscreen to trigger them — they're documented as follow-ups. **Waves 3-11** remain open: app install, case list/forms, multi-app, marketplace, sync, settings, edge cases, reliability. See `docs/superpowers/specs/2026-04-08-phase9-e2e-ui-testing-design.md`.

**If you are an agent starting a session:** Phase 9 is the active phase. Waves 0-2 are complete. If continuing Phase 9, read `docs/phase9/fixture-user.md` for the orchestrator workflow, then pick the next wave from the spec §7 summary. Wave 3 (app install) is the next natural step.
