...
**Phase 9: End-to-End UI Testing — Waves 0-4b COMPLETE, Waves 4c-11 open.** Distinct successor to Phase 8 that closes the UI-level E2E gap AND picks up the relocated Phase 8 live-Connect test tail. The `+7426` Connect ID test prefix is used with a Playwright-driven OTP fetch against `https://connect.dimagi.com/users/connect_user_otp/` (persisted Dimagi SSO session) — the originally-planned OAuth2 client credentials were never needed. **Wave 0** (infrastructure + relocated Phase 8 live-Connect tests), **Wave 1** (Connect ID recovery flow), **Wave 2** (post-recovery state + session persistence), **Wave 3** (CommCare app install + login), **Wave 4a** (module list → case list navigation), and **Wave 4b** (form navigation + back, no submit) are all shipped and reproducible via `.maestro/scripts/run-*.sh` orchestrators. Fixture user `+74260000042` (Hal Test) is registered in connect-id prod. Wave 3+ installs `Bonsaaso Application` from `jonstest` domain on HQ prod and logs in as `haltest@jonstest.commcarehq.org`. Product bugs caught and fixed during Phase 9: keychain bridging crash (Wave 0), recovery-endpoint misuse in new-reg flow (Wave 0), empty-photo rejection (Wave 0) — see `docs/learnings/2026-04-08-phase9-ios-connect-id-bugs.md`. Bugs caught and filed (not yet fixed): state inconsistency between SetupScreen and Connect menu post-recovery (#389, Wave 2); `LoginViewModel.resolveDomain()` falls back to hardcoded `"demo"` for short-form usernames (#391, Wave 3); can't type into constraint-validated text form fields because `FormEntryViewModel.answerQuestion` never updates UI state on constraint violation (#394, Wave 4b — blocks Wave 4c). Wave 2 deferred PIN login + biometric tests because iOS has no lockscreen to trigger them. **Waves 4c-11** remain open: full form submit (blocked on #394), multi-app, marketplace, sync, settings, edge cases, reliability. See `docs/superpowers/specs/2026-04-08-phase9-e2e-ui-testing-design.md`.

**If you are an agent starting a session:** Phase 9 is the active phase. Waves 0-4b are complete. If continuing Phase 9, read `docs/phase9/fixture-user.md` for the orchestrator workflow (both Connect ID and HQ test app), then pick the next wave from the spec §7 summary. Wave 4c (full form fill + submit + HQ verify) is blocked on #394 — fix that bug first, then ship 4c. Alternative next step: jump to a different wave like W6 (multi-app management) that doesn't depend on form entry.
