...
**Phase 9: End-to-End UI Testing — COMPLETE (Waves 0-9).** 15 bugs found and fixed, 34 JVM regression tests added, 63 placeholder tests removed, 9 Maestro E2E flows + 3 orchestrators shipped. See `docs/plans/2026-04-10-phase9-completion-report.md`. Key findings: (1) iOS keychain `SecItemAdd` silently fails in Compose context — worked around via SQLite DB fallback; (2) CMP iOS `mutableStateOf` mutation during composition doesn't trigger recomposition — fixed by moving state bridges to `LaunchedEffect`; (3) The ViewModel layer had zero real tests — placeholder tests (`assertFalse(false)`) gave misleading green CI. W7 Connect marketplace partially tested (opportunities list + detail work; messaging consent blocked on server-side channel configuration). W10-W11 deferred to device testing (need real iOS hardware for network loss, permissions, lockscreen).

**If you are an agent starting a session:** Phase 9 is complete. The next phase should focus on: (1) backfilling ViewModel test coverage, (2) investigating the iOS keychain `SecItemAdd` failure root cause, (3) App Store submission (remaining manual steps in `project_appstore_remaining.md`). Read `docs/plans/2026-04-10-phase9-completion-report.md` for the full summary.
...
- Phase 9 (E2E UI Testing, 15 bugs, W0-W9): `docs/plans/2026-04-10-phase9-completion-report.md`
...
- **Phase 9 E2E**: `ios-xml-whitespace-coalescing-gap` (iOS XML parser divergence), `viewmodel-layer-coverage-gap` (ViewModel has no real tests, placeholder audit)
