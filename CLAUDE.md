...
**Phase 9: End-to-End UI Testing — Waves 0 and 1 COMPLETE, Waves 2-11 open.** Distinct successor to Phase 8 that closes the UI-level E2E gap AND picks up the relocated Phase 8 live-Connect test tail. The `+7426` Connect ID test prefix is used with a Playwright-driven OTP fetch against `https://connect.dimagi.com/users/connect_user_otp/` (persisted Dimagi SSO session) — the originally-planned OAuth2 client credentials were never needed. **Wave 0** (infrastructure + relocated Phase 8 live-Connect tests) and **Wave 1** (Connect ID recovery flow) are both shipped and reproducible via `.maestro/scripts/run-registration.sh` and `.maestro/scripts/run-recovery.sh`. Fixture user `+74260000042` (Hal Test) is registered in connect-id prod and verified end-to-end. Three real iOS product bugs were caught during Wave 0 execution and fixed (keychain bridging crash, recovery-endpoint misuse in new-reg flow, empty-photo rejection) — see `docs/learnings/2026-04-08-phase9-ios-connect-id-bugs.md`. **Waves 2-11** remain open: login variants, app install, case list/forms, multi-app, marketplace, sync, settings, edge cases, reliability. See `docs/superpowers/specs/2026-04-08-phase9-e2e-ui-testing-design.md`.

**If you are an agent starting a session:** Phase 9 is the active phase. Waves 0 and 1 are complete. If continuing Phase 9, read `docs/phase9/fixture-user.md` for the orchestrator workflow, then pick the next wave from the spec §7 summary. Wave 2 (login variants — PIN set, PIN login, biometric enroll, session restore) is the next natural step.
