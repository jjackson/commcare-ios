 ...
 */
interface QuerySetLookup {

    /**
     * Checks the current evaluation context to see if this lookup can establish a reference
     ...
     * IE: instance('casedb')/casedb/case[15]
     *
     * which has a specific semantic meaning to this lookup and can be optimized
     *
     * @return null if this lookup can not identify elements in the provided context. Otherwise
     * a "key" tree reference to be used by the other methods in this class
     */
    fun getLookupIdKey(evaluationContext: EvaluationContext): TreeReference?
     ...
     */
    fun isValid(lookupIdKey: TreeReference?, context: QueryContext): Boolean

    /**
     * A semantic value identifying the "model type" that is being returned by this
     ...
     * model.
     */
    val queryModelId: String

    /**
     * A key that identifies the Model Query Set this lookup will use, and should used to
     * identify/resolve the query set in the current context.
     *
     * EXAMPLE: "case_current" <- the set of cases which are returned by current()
     */
    val currentQuerySetId: String
     ...
     */
    fun performSetLookup(lookupIdKey: TreeReference, queryContext: QueryContext): List<Int>?
     ...
     */
    fun getLookupSetBody(queryContext: QueryContext): Set<Int>
}
