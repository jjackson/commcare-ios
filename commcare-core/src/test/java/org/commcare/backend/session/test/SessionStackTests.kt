 ...
 */
class SessionStackTests {

    @Test
    fun testDoubleManagementAndOverlappingStack() {
        val mockApp = MockApp("/complex_stack/")
        ...
        val actions = session.getDetail(entityDatum.getShortDetail())!!.getCustomActions(
    ...
    fun testViewNav() {
        ...
        CaseTestUtils.xpathEvalAndCompare(
            session.getEvaluationContext(),
    ...
    @Test
    fun testOutOfOrderStack() {
        val mockApp = MockApp("/session-tests-template/")
        val session = mockApp.getSession()
    ...
    @Test
    fun testOutOfOrderStackComplex() {
        val mockApp = MockApp("/session-tests-template/")
        val session = mockApp.getSession()
    ...
    @Test
    fun testUnnecessaryDataOnStack() {
        val mockApp = MockApp("/session-tests-template/")
        val session = mockApp.getSession()
     ...
     */
    @Test
    fun testNonCaseInstanceOnStack() {
        val mockApp = MockApp("/session-tests-template/")
        val session = mockApp.getSession()
        ...
        val dataInstance = buildRemoteExternalDataInstance(
            this.javaClass, session,
            "/session-tests-template/patient_query_result.xml"
        )
        session.setQueryDatum(dataInstance)

        ExprEvalUtils.testEval(
            "instance('patients')/patients/patient[@id = '321']/name",
            session.getEvaluationContext(),
            "calbert"
        )
    }
    ...
    fun testCaseInstancesOnStack() {
        ...
        ExprEvalUtils.testEval(
            "instance('patients')/patients/case[@id = '123']/name",
            session.getEvaluationContext(),
            "bolivar"
        )
        ...
        assertInstanceMissing(session, "instance('patients')/patients/case/bolivar")
        ExprEvalUtils.testEval(
            "instance('session')/session/data/case_id",
            session.getEvaluationContext(),
            "bolivar"
        )
    }
    ...
    fun testActionParsing() {
        val mApp = MockApp("/complex_stack/")
        ...
        val ec = session.getEvaluationContext()
        val actions = session.getDetail(entityDatum.getShortDetail())!!.getCustomActions(ec)
    ...
    companion object {
        @JvmStatic
        fun buildRemoteExternalDataInstance(
            cls: Class<*>,
            sessionWrapper: SessionWrapper,
            resourcePath: String
        ): ExternalDataInstance {
            val remoteQuerySessionManager = RemoteQuerySessionManager.buildQuerySessionManager(
                sessionWrapper,
                sessionWrapper.getEvaluationContext(), ArrayList()
            )
            val inputStream: InputStream = cls.getResourceAsStream(resourcePath)
            val instanceOrError =
                remoteQuerySessionManager!!.buildExternalDataInstance(
                    inputStream, resourcePath, ListMultimap.create()
                )
            assertNotNull(instanceOrError.first)
            return instanceOrError.first!!
        }

        private fun assertInstanceMissing(session: SessionWrapper, xpath: String) {
            try {
