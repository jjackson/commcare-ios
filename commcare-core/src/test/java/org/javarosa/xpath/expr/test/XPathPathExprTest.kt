 ...
 */
class XPathPathExprTest {

    @Test
    fun testHeterogeneousPaths() {
     ...
     */
    @Test
    fun testNestedMultiplicities() {
        val fpi = FormParseInit("/test_nested_multiplicities.xml")
        ...
        ExprEvalUtils.testEval("join(' ', /data/bikes/manufacturer[@american='yes'][count(model=1) > 0]/model/@id)",
                fd.getInstance(), null, XPathTypeMismatchException())
    ...
    fun testNestedPreds() {
        ...
        val fd = fpi.getFormDef()!!
        val groupsInstance = fd.getNonMainInstance("groups") as FormInstance
        val ec = fd.getEvaluationContext()
    ...
    @Test
    fun testCaseDbQueriesFromForm() {
        val fpi = FormParseInit("/test_casedb_query_from_form.xml")
        val fd = fpi.getFormDef()!!
        val casedb = fd.getNonMainInstance("casedb") as FormInstance
        val ec = fd.getEvaluationContext()
