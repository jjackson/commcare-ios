 ...
 * to evaluate them, and provide an expectation of how efficient they will be in doing so, to
 ...
 * a predicate, but a match turns out to not be possible. In those cases it's still important to be
...
interface QueryHandler<T> {
     ...
     * limitations in how we can prevent lengthy operations from occurring when generating them.
     *
     ...
     * can return null, which will signal that the query couldn't be run and no predicates have
     * been evaluated.
     */
    fun loadProfileMatches(querySet: T, queryContext: QueryContext): List<Int>?

    /**
     * Given a successful profile match, this method updates the predicate profiles to remove profiles
