 ...
 */
interface EntityLoadingProgressListener {

    enum class EntityLoadingProgressPhase(@JvmField val value: Int) {
        /**
        ...
        companion object {
            @JvmStatic
            fun fromInt(i: Int): EntityLoadingProgressPhase {
                for (phase in entries) {
                    if (phase.value == i) {
                        return phase
                    }
                }
                throw IllegalArgumentException("Unexpected value: $i")
            }
        }
    }
     ...
     */
    fun publishEntityLoadingProgress(phase: EntityLoadingProgressPhase, progress: Int, total: Int)
}
